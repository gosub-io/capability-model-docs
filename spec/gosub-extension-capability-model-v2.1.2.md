...
- `filtering.rewrite_url` is **not** a sink only because it is deletion-only (§6): the rewritten URL is the original with query parameters removed and nothing else. Substitution-based rewriting — even "same-origin only" — *is* an `arbitrary_network` sink, because a same-origin open redirect (`victim/redirect?to=evil/?d=<captures>`) launders rule-derived bytes to any host. Gosub therefore offers no substitution rewrite at any tier below `filtering.redirect_*`'s static-target rule.
- `filtering.scriptlet` (browser-supplied library) is `source: none, sink: none, actuator: dom`: the injected code is browser code, no parameter is ever *loaded* as a resource, and there is no channel from the injected realm back to the extension. Extension-authored scriptlets are `page.main_world_inject` and carry that entry's labels.
...
**URL rewriting is deletion-only.** `filtering.rewrite_url` lets a rule strip query parameters from a matched request before connect (`$removeparam=utm_source`, DNR `queryTransform.removeParams`) — the tracking-parameter case. The invariant that keeps it out of the sink lattice:

```text
rewritten.scheme/host/port/path  ==  original's, byte for byte
rewritten.query                  ==  original.query minus the matched
                                     keys (by name, or by linear-time
                                     regex over key/value); the whole
                                     query may be dropped
insertion / substitution / reorder   never — no captures, no templates,
                                     no rule-derived bytes anywhere
```

Because the result is always a subsequence of a URL the page itself produced, no request can be steered to an extension-chosen destination and no page state can be encoded into a request. Every other transform a list language offers — path/host/scheme changes, `regexSubstitution`, `$urltransform` with a replacement — is a redirect with rule-derived components, which the previous paragraph forbids; those rules are dropped at compile time with the same downgrade posture as unsupported modifiers, never approximated. Like `filtering.block`, a rewrite is unobservable to the extension: no event, no counter outside the §7 namespace.
...
parameters    data only — strings, numbers, selectors, match patterns.
              No parameter is ever LOADED: nothing designates a resource
              to fetch, navigate to, or inject; nothing is evaluated as
              code. Match-against-URL is fine (prevent-fetch); a URL to
              load is not (no href-sanitizer-to-arbitrary, no
              trusted-* family, no set-cookie/replace-response with
              attacker-chosen bytes)
channel       NONE from the injected realm back to the extension — no
              runtime port, no shared storage, no observable side
              effect the worker can read; scriptlets change the page,
              they never report on it
budget        per-operator cost bound as for procedural filters
```

That keeps `filtering.scriptlet` at `source: none, sink: none` (§5) and standard tier: it is page-integrity power (I:high — it changes what the page's own scripts see) with no confidentiality path, and it lifts the last routine reason a blocker needs `content_script`. Anything not in the library — a custom scriptlet, a `trusted-*` operator, general code — is `page.main_world_inject` (loud) or `content_script`, per site, as before. Surrogates (§6, response replacement) and scriptlets (document-start injection) are two delivery mechanisms over the same audited library.
...
filtering.rewrite_url(class, hosts)   subresource silent / main_frame
                                      standard; DELETION-ONLY — removes
                                      query parameters (by name or linear-
                                      time regex), scheme/host/port/path
                                      byte-identical; no insertion or
                                      substitution at any tier (§6);
                                      unobservable like block;
                                      source: none, sink: none
filtering.scriptlet(hosts)            standard; browser-supplied audited
                                      library ONLY; rules select +
                                      parameterize by name; parameters are
                                      data — never loaded as a resource,
                                      never evaluated as code; injected by
                                      the browser at document-start into the
                                      page main world; NO channel back to
                                      the extension; cost-bounded (§8);
                                      source: none, sink: none, actuator:
                                      dom. Extension-authored scriptlets
                                      are page.main_world_inject.
...
Still open:
  ...
  O3  The exact procedural DSL operator set at launch (criterion in §8),
      ...
      surrogate library (criterion in §8: no parameter loaded, no
      channel back; the trusted-* family is out by construction).
