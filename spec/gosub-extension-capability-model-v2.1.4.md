...
matching          on a NORMALIZED SCRATCH COPY — percent-decoded,
                  '+'-to-space, '&' and ';' both separators — so a
                  key the server will read as `fbclid` is matched
                  whether the page wrote `fbclid`, `%66bclid`, or
                  `fbclid ` (raw-byte matching would let an encoded
                  tracker slip the filter); linear-time regex over the
                  decoded key/value; a matched key marks ALL its
...
responses     FIXED ENUM (empty object / array / string, noop js) —
              ...
              no trusted-* family)
channel       the boundary is BYTE PROVENANCE, not observability. A
              scriptlet's DOM effects are observable to a co-resident
              cosmetic engine or content script by construction — that
              is the scriptlet's job — so "no observable effect" would
              be vacuous. The real invariant: no operator ever WRITES A
              VALUE DERIVED FROM PAGE STATE IT READ. Writing a rule-
              supplied constant is fine (a content script seeing
              `x===false` learns nothing about the user); copying a
              json-prune'd field, a matched request URL, or any read
              page datum into the DOM, a global, or storage is
              forbidden, because that is the source→extension channel
              in disguise. Scriptlets change the page with rule-
              supplied data; they never launder page-derived data out
...
filtering.scriptlet(hosts)            standard; browser-supplied audited
                                      ...
                                      §8); write targets an ALLOWLIST not
                                      denylist; per-operator arity/type
                                      schema; cost-bounded against page
                                      input; RE-VALIDATED renderer-side
                                      against the untrusted artifact (§14);
