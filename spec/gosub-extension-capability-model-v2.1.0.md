...
**Scopes are canonical.** Every host/origin scope is normalized at grant-generation time through the *same* normalization library the network layer uses at match time — one shared crate for IP, IPv6, `localhost`, and eTLD+1 forms. Scopes are stored canonical, never as raw manifest strings, so the translator and Sonar can never disagree about whether `[::1]`, `localhost`, and `127.0.0.1` are the same origin. A scope is only as sound as the single canonicalizer both sides trust.
...
**`stats.read` (standard).** Returns **exactly one dimensionless counter**: total blocks across all rules across all sites, on a coarse schedule. No per-rule, per-site, or per-window breakdown — those are `stats.per_rule` (loud). This closes the frequency-domain oracle: 10 000 single-domain rules all increment the same counter, so their time series carries no per-site signal. Any internal per-rule counters (for list-maintainer diagnostics) live in a namespace no standard-tier extension can read, even rounded.

The defense rests on **value quantization and single-dimensionality, never on timing decorrelation** — `scheduling` (silent) supplies precise clocks, so any timing-based decorrelation is assumed defeated. When any probe sink (`filtering.dynamic_rules`, `filtering.remote_rulesets`) is co-held, even the single counter degrades further or is denied, and the composition marks the pair as derived observation.
...
Content-hash pinning is a requirement, not an option: browser-fetching removes extension-controlled personalization inputs but not the server's ability to vary the response by client IP/UA. Pinning is what actually guarantees every user runs the same reviewed list; the stronger form is a transparency log or shared-cache proxy. An extension that instead fetches rules itself and installs them via `dynamic_rules` is doing per-user policy, and the composition treats that `egress × dynamic_rules` pair as remote control (§5).
...
uBlock-class blocker wants to:
    ...
    ✗ It cannot be controlled by a remote server
...
network.proxy_control                 gated      persistent indicator;
                                      command actuator: os/navigation
...
dom.declarative_actions(hosts)        standard  I:high; selectors declared at
                                             install; never mints activation;
                                             state-changing/form targets on main
                                             frame take the main-frame tier
...
Still open:
  ...
  O6  Signing / trust mechanism (a UA concern per §17).
