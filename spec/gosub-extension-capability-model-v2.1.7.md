...
Axis 1 defines a product for **every** source atom, not the original six: `aggregate × sink -> aggregate.exfiltration`, `selection × sink -> selection.exfiltration`, `download_urls × sink -> history.exfiltration`, `user_text × sink -> query.exfiltration`, `browser_traffic × sink -> traffic.exfiltration`, `session_state × sink -> session.exfiltration`. The vocabulary is closed and the validation is a build step, not prose.
...
**The shared page realm is a browser-provided channel, and the closure treats it as one.** A red-team pass showed a gap: extension A (publisher P1) holds `filtering.scriptlet` or `content_script` and writes `window.__x = pageData`; extension B (publisher P2) holds `content_script + egress` and reads `window.__x` and exfiltrates it. This is *not* the "own servers + OS covert channel" residual (that genuinely is out of scope) — the main world and the shared page DOM are an execution environment **the browser itself provides**, so two extensions that both reach a given page's main world / shared DOM are a communicating set for that page, and their source×sink authority composes there. Because enumerating every such cross-publisher pair at install time is infeasible, the channel is *narrowed* rather than closed: scriptlet/content-script write surfaces get per-document, per-principal randomized property namespaces so a receiver cannot predict where to read, and both `filtering.scriptlet` and `content_script` carry a loud tier whose dialog states page access plainly. The residual — determined cross-publisher collusion through the page realm despite randomization — is named in O7, and it is a standing reason `filtering.scriptlet` does not drop below loud casually. Unrelated publishers colluding purely through their own servers plus an OS covert channel remain out of manifest-analysis scope, handled by store-side signals.
...
transitive    and the reverse direction too: an operator's write must
              ...
              it, so this is a sink. A generic set-constant(path,
              value) therefore cannot be proven sink:none from target
              shape + value vocabulary alone; the O3 proof must be a
              TRANSITIVE-effect proof, or the standard-tier operator set
              is restricted to strictly SUPPRESSIVE/monotonic semantics
              (only ever disables or no-ops a page behavior, never
              enables one). Until such a proof exists the operator is
              dom power with a possible page-mediated sink, which is
              the standing reason the tier stays loud (§11, §19)
...
filtering.remote_rulesets (model C — see below; NOT a package-pinned hash):
    ...
    freshness:catalog carries a signed monotonic revision + max-age;
              a client past max-age that cannot get the current
              approved revision FAILS the fetch (no silent stale)
...
- **Targeted staleness is an attack, so freshness is enforced.** A content-addressed fetch stops a server serving *different accepted bytes* per user, but not a server *withholding* the newly-approved revision from selected clients to keep them on an old policy. The catalog therefore carries a signed monotonic revision number with a max-age; the browser refuses to keep using a revision past its max-age and treats a server that cannot supply the current approved revision as a fetch failure, not as a silent fall-back. Where the threat model demands it, the catalog/store mirrors the bytes so the publisher's server is not the sole distributor.
...
**One extension never filters another extension's traffic.** Sonar attributes every request to an initiator principal; extension filtering tables apply to *page* traffic, never to requests initiated by another extension principal. Without this, blocker A could block or rewrite password-manager B's sync/API traffic — a cross-extension integrity/availability power and a feedback channel outside the §5 publisher closure. **Protected traffic** (browser/extension updates, certificate validation, browser-internal services, another extension principal's requests, `gosub://`) is never subject to extension filtering. Sonar enforces all of this — the check must sit where the connection is made.
...
filtering.remote_rulesets(catalog)    standard; browser-fetched, MODEL C
                                      ...
                                      mismatch or stale; NOT a per-package
...
tabs.open / tabs.navigate             standard  sink: arbitrary_network +
                                             ...
                                             ALLOWLIST — web schemes only;
                                             javascript: is NEVER accepted
                                             (it would be main-world execution,
                                             not navigation); file:/internal/
                                             extension schemes require separately
                                             named authority
...
omnibox.navigate                      standard  actuator: navigation +
                                      sink: arbitrary_network; same scheme
                                      allowlist as tabs.navigate (no
                                      javascript:/internal schemes)
...
Still open:
  ...
  O3  The exact procedural DSL operator set at launch (criterion in §8),
      ...
      — or restriction to strictly suppressive/monotonic semantics —
      because even a vocabulary-safe write can drive a page gadget,
      §8); the trusted-* family is out by construction. The semantic
  ...
  O5  stats privacy-budget parameters (criterion: a single-site probe
      needs O(weeks) to distinguish one visit from noise), enforced
      against a GLOBAL per-profile pool, partitioned regular/private
  ...
  O7  Covert-channel review (storage/cache/DNS/timing/quota) — the one
      ...
      scripts (§8) — now treated by §5 as a browser-provided
      communicating set and NARROWED by per-document per-principal
      property-name randomization plus the loud tiers, so the residual
      O7 owns is determined collusion that defeats randomization, not
      the whole channel — and rewrite_url's destination-server
      observability (§6).
...
Remaining: two loud grants, both revocable per site — content_script
           ...
           the O3 per-operator proofs relax it to standard. Still no
