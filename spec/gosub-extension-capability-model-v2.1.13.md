...
The mutation form matters because it needs no new request to leak: a filter rule that fires *conditionally on a predicate the observer cannot otherwise see* and mutates traffic to a server encodes that predicate into what the server observes — the rule acquired the fact and the mutation emitted it — even with no `network.observe`, `stats.read`, or extension egress in the grant. This generalizes what §6 already conceded for `rewrite_url` (the destination server can observe stripping): the concession was not special to rewriting, it is the rule for all outbound mutation — **including presence/absence**: a blocked-vs-allowed request is the most basic externally-observable outbound mutation, so `filtering.block`/`filtering.allow`/`upgrade_scheme` are inside this rule, not outside it (a general block plus an exception `@@||publisher-cdn.example^$domain=clinic.example` delivers a request to the publisher's server *only* from clinic.example).

v2.1.9 stated the consequence as "standard-tier mutations must be unconditional on initiator/party/page-derived predicates". That was **mis-scoped in both directions** and is replaced by a precise **leak-free criterion**. A mutation (set/remove header, strip parameter, block, allow, upgrade, redirect) is leak-free — and therefore `source: none` for the extension — iff **both** hold:

```text
(1) PREDICATE   every fact the rule's predicate depends on is one the
                OBSERVER of the mutation already holds. For a request
                mutation the observer is the destination server, which
                already sees the request URL, method, the headers as
                sent, and the PARTY relation (Sec-Fetch-Site) — so a
                predicate over the request URL or over $third-party /
                domainType is leak-free, and the initiator origin is
                leak-free wherever the request itself carries it
                (Referer / Origin under the effective referrer policy).
                NOT already held, and therefore a leaking predicate:
                the TOP-FRAME identity for a request made from a nested
                cross-origin frame ($domain= in uBO matches the top
                document, which the frame's server cannot see under
                partitioned state); the initiator on a referrer-
                suppressed request; any fact about OTHER requests,
                tabs, history, or page state; and — for a mutation
                whose effect is observed by a DIFFERENT server than the
                one whose message is mutated (a response header that
                makes the UA fetch elsewhere, §15) — the request URL
                itself, because that third server never saw it.
(2) INSTALL     the rule reached the engine through a channel the
                WORKER does not choose at runtime: the package, a
                catalog revision (§9), or text the user typed. A rule
                the worker installs at runtime (filtering.dynamic_rules)
                fails this clause REGARDLESS of how unconditional the
                rule looks, because the worker encodes its own state in
                WHICH rule it installs (below).
```

A mutation failing (1) composes as a source (`implicit_history` for top-frame/initiator facts, `page_content` for page-derived facts) and is tiered loud, exactly as `network.observe` would be; the compiler can decide clause (1) per rule from the predicate's field list, so a list that mixes leak-free and leaking rules is split at compile time and only the leaking residue needs the loud grant (§18). The old "no initiator/party predicate" wording would have made `$3p,removeparam=`, `$removeheader` with `$domain=`, and DNR `initiatorDomains` untranslatable at standard tier for no gain; the precise criterion keeps them standard where the destination already sees the fact.

**Clause (2) is why `filtering.dynamic_rules` is an outbound sink.** Install-time rules carry no worker-chosen bit. Runtime-installed rules do: the worker picks header-rule variant *k* of *N* (`X-Hint: <static-k>`) and a publisher-observed server learns *k*; or — with nothing but silent `filtering.block` — blocks/unblocks `publisher-cdn.example/beacon.gif` and the server sees presence/absence, one bit per update, on any page that loads a publisher-observable resource or whenever the user visits a publisher page. Low bandwidth, unbounded over time, no egress grant, no content script. So `dynamic_rules` is `sink: own_hosts` (a *rule-choice channel* to every server the publisher can observe from page traffic), in addition to `sink: probe`; `tab_urls + dynamic_rules` therefore derives `history.exfiltration` on Axis 1 where v2.1.9 derived nothing. (The `source: implicit_history` label v2.1 put on `dynamic_rules` — "a single-URL dynamic rule plus a timing loop turns the matcher into a navigation detector" — is **withdrawn**: no worker-observable channel was ever described for it — packaged loads are unobservable, cache state is isolated, there are no events and no readable counters — and an asserted channel with no mechanism only produced a false derived warning on the flagship blocker (§11, Appendix C). `sink: probe` already captures the real property: a dynamic rule *targets* whatever other source the set holds.)
...
source label   none | aggregate | tab_urls | page_content |
               ...
               browser_traffic | installed_extensions |
               account_identity | device_fingerprint
...
command-source command authority INTO the extension's decisions
               ...
               catalog_revision | remote_server | webpage |
               native_process | enterprise_policy
...
Axis 1 defines a product for **every** source atom, not the original six: `aggregate × sink -> aggregate.exfiltration`, `selection × sink -> selection.exfiltration`, `download_urls × sink -> history.exfiltration`, `user_text × sink -> query.exfiltration`, `browser_traffic × sink -> traffic.exfiltration`, `installed_extensions × sink -> install.exfiltration`, `account_identity × sink -> identity.exfiltration`, `device_fingerprint × sink -> fingerprint.exfiltration`. **`device_fingerprint` is the machine, not the person**, which is why it is neither `account_identity` nor `aggregate`: it identifies a browser installation with no account involved, and it names concrete attributes rather than the dimensionless counts `aggregate` covers. Its danger is **linkability**, and specifically linkability ACROSS the boundaries the rest of the model works to keep apart — a fingerprint read in a private-browsing context is the same fingerprint read outside it (§12), and one read today is the same one read after every cookie in the profile has been cleared. That is also why no single reader looks alarming and the atom still earns its place: screen geometry, hardware profile and the installed font list are individually standard-tier and jointly a stable identifier, so the composition is exactly what the §5 closure exists to surface rather than something any one dialog line could say. **`account_identity` is the signed-in human, not a secret**, which is why it is its own atom rather than a use of `credentials`: an email address and a stable account id authenticate nothing, so labelling them `credentials` over-warns on the confidentiality axis while *under*-stating the real property — an account identifier is the join key that makes every other source in the set attributable to a named person. `tab_urls` alone is a browsing history; `tab_urls + account_identity` is *this person's* browsing history, and the closure should say so. `session_state` is deliberately **not** among them: it is a sink and an actuator, never a source — a written cookie *carries* data out, it does not *acquire* any — so its Axis-1 role is on the sink side (`page_content × session_state-sink -> session.exfiltration`) and a `session_state × sink` product would be ill-typed. (An earlier draft listed exactly that; the type discipline the build-step validation enforces is what caught it.) The vocabulary is closed and the validation is a build step, not prose.
...
- `filtering.dynamic_rules` is `sink: probe + own_hosts` (and `source: none` — see the leak-free criterion above): a dynamic rule *targets* whatever other source the set holds (`stats.read` becomes a per-site counter), and the worker's runtime *choice* of rule is itself an outbound channel to publisher-observable servers.
...
remote_server × session_state    ->  remote.session_control
remote_server × os               ->  remote.os_control
remote_server × extension_bridge ->  remote.bridge_control
webpage       × {actuator}       ->  page.{actuator}_control
                                     (page.os_control is the web →
                                     extension → native-host chain)
native_process × {actuator}      ->  native.{actuator}_control
enterprise_policy × {actuator}   ->  policy.{actuator}_control
                                     (admin-driven; shown as managed,
                                     not as a warning)
catalog_revision × filter_policy ->  catalog.filter_control
                                     (trusted anchor, §9 — named so
                                     catalog compromise is a stated
                                     residual, O9, not an invisible one)
publisher_update × {actuator}    ->  publisher.{actuator}_control
                                     (every auto-updating extension;
                                     governed by the §13 diff, not
                                     shown per install — §11)
```

Axis 2 is closed the same way Axis 1 is: a product exists for **every** `command-source × actuator` pair, named by the scheme above, and the build validator rejects an entry whose command-source or actuator has no product. v2.1.9 listed five products and left `remote_server × os` (every `actuator: os` entry), `remote_server × session_state` (named only in prose), `native_process × *` ("etc."), `webpage × *`, `enterprise_policy × *`, `publisher_update × *` undefined while claiming "no product is left undefined" — and carried the atoms `webpage`, `extension_bridge`, `publisher_update` on no registry entry at all. Every atom is now carried by at least one entry (§19).
...
**Any channel that delivers remotely chosen bytes to extension code is a `remote_server` command-source.** The earlier derivation ("egress-to-own-host + a *mutation* capability") was too narrow: a mutation capability is only needed when the thing being remotely driven is *declarative policy*. For a directly callable actuator no mutation step is required — `network.egress_public(["c2.example"]) + tabs.navigate` lets the worker poll `c2.example`, receive `{url}`, and call `tabs.navigate(url)`, which is plainly `remote_server × navigation -> remote.navigation_control`. The same holds for `downloads.create`, `ui.notifications`, `dom.actions_arbitrary`, `cookies.write`, `omnibox.navigate`. v2.1.6–v2.1.9 stated the rule as "any egress the worker can read a response from"; that is still **egress-centric**, and a red-team pass listed readable remote channels that carry no `network.egress_*` grant at all:

```text
readable egress          fetch / WebSocket / WebTransport response bytes
messaging.external_web   a WEB PAGE posting to the extension
                         (externally_connectable with web origins,
                         runtime.onMessageExternal) — command-source:
                         webpage; the page is remote-chosen content
messaging.push           a push service delivering bytes to the worker
                         with no egress grant in the set
content_script(hosts)    whenever `hosts` contains an origin the
                         publisher can control, the page DOM is a
                         publisher-writable, extension-readable channel:
                         the user visits publisher.example and the
                         content script reads its instructions. Weaker
                         (user-visit-gated) but real; `<all_urls>`
                         always contains such an origin
remote iframes           an extension page (newtab override, popup)
                         embedding https://publisher.example/… — the
                         frame posts messages back
```

So the rule is: **any channel through which remotely chosen bytes reach extension code is a `remote_server` command-source (or `webpage`, where the sender is a page), and it composes with every actuator the same set holds.** `content_script` and `page.main_world_inject` therefore carry `command-source: webpage` (the page can drive them — the MetaMask shape), and the registry gains `messaging.external_web` and `messaging.push` entries (§19) so the web-page → extension → native-host chain (`webpage × os`) is derivable rather than invisible. `dynamic_rules` (or another mutation capability) is additionally required *only* to drive `filter_policy`, because filtering is declarative rather than a callable API. This widens Axis 2 well beyond the filtering case and is why the detector composes over the whole actuator column, not a pair table.

Axis 2 is a **derived-pattern detector**, not a per-capability tier: `command-source: remote_server` is emergent (any readable remote channel), while `command-source: native_process` / `enterprise_policy` / `publisher_update` and every `actuator` *are* per-capability labels the registry (§19) assigns. Both axes are computed the same way — a closure over labels — but axis 2's remote-server source arises from a combination rather than a single grant. Every registry entry that can drive filter policy, navigation, the DOM, browser UI, or the OS carries an `actuator` label so the detector has something to compose against; entries with none are inert to axis 2.
...
**Redirect targets are static and classed, and a redirect is a response substitution, not an HTTP redirect.** A redirect rule targets only a resource from the extension's fixed packaged set — no rule-derived components, no regex substitution, no fragments, no query propagation. The engine serves the packaged bytes **in place**: the request's `response.url` stays the original URL, no 30x is emitted, and no extension-origin URL ever reaches the page — otherwise `fetch().url` would hand the page the per-session-randomized `web_accessible_resources` token and the randomization would buy nothing (the page fingerprints the installed extension). Passive targets (image, empty response, static text) are `filtering.redirect_resource`. **Executable surrogates split by what they must reach.** A surrogate that only needs to *exist* (`noop.js`, an empty module) runs in an **isolated realm** with no access to page `localStorage`, `sessionStorage`, or `postMessage` to page origins, under a no-network CSP — because a surrogate executing in the page could otherwise launder exfiltration through page JS (`window.app.send(secret)`), a cooperating frame, or DOM-local channels a same-origin content script reads. But the surrogates that *justify* the category — `googletagservices_gpt.js`, `google-analytics_ga.js` — exist precisely to **define page globals the page's own code calls** (`window.googletag`, `window.ga`); an isolated-realm stub cannot do that, and an earlier draft's "surrogates run in an isolated realm and cannot patch page globals" was therefore hollow for exactly the entries that matter. API-stub surrogates are **main-world code** and are admitted only as members of the §8 scriptlet library, under its full admission closure and its loud-until-O3 tier; a membrane that exposes isolated-realm objects to the page was considered and rejected (page callbacks crossing it reopen every sink question the realm was meant to close). Browser-supplied audited surrogates are preferred and stay lower-tier; extension-authored executable surrogates are treated as `page.main_world_inject`-class (loud).
...
**Cosmetic mutation is observable in the page, and that is fine — because every extension context that could observe it already holds `page_content`.** Earlier drafts required hiding to be "compositor-level, invisible to `MutationObserver` and to any content script" and relabelled `filtering.cosmetic` `source: page_content` otherwise. That requirement was both impossible and unnecessary. Impossible: cosmetic filtering *collapses* (`display:none` semantics — the page reflows and the ad's space disappears); a post-layout compositor hide leaves a blank hole and is not what the capability is for, and any collapse is visible through `getBoundingClientRect`, `IntersectionObserver`, `offsetParent`, `checkVisibility()` regardless of `MutationObserver`. Unnecessary: the feared channel — `##div:has-text("secret"):upward(1)` plus a poll — needs an **extension-side observer**, and the only extension contexts that can observe page layout (`content_script`, `styles.read`, `devtools.dom`) already carry `source: page_content` and can read the secret directly; page JS observing the hide is the *page* learning about the page. So `filtering.cosmetic`/`procedural` stay `source: none` with no compositor caveat; the one residual — a *different* publisher's content script observing this extension's cosmetic effects — is the shared-realm channel O7 already owns, and hiding is a per-user ruleset fingerprint to sites that look, the same residual §6 names for `rewrite_url`.
...
filtering.remote_rulesets (model C — see below; NOT a package-pinned hash):
    ...
    pin:      the package pins the CATALOG identity + signing key;
              ...
              A catalog is a UA/STORE-TRUSTED ANCHOR — the package may
              only name a catalog the UA trusts; a publisher-run key is
              not a catalog (that is model B)
    log:      every revision is an entry in an APPEND-ONLY transparency
              log; the browser accepts a revision only with an
              INCLUSION proof and a CONSISTENCY proof against its last
              seen tree head (or a threshold of independent WITNESS
              co-signatures on that head) — signed+monotonic alone
              does not stop a split view
...
Gosub adopts **model C**, and the earlier "MANDATORY hash embedded in the package" wording (which is model A) is corrected to match: the package pins a **catalog identity and the catalog's signing key**, not a single frozen content hash. The approver — a UA/store catalog, not the publisher's server — publishes signed `(version, hash)` revisions; the browser fetches exactly the approved immutable object and verifies it against the *catalog-signed* hash, so lists update through newly-approved revisions *and* every user runs the same reviewed bytes. **"Provably" the same bytes needs a mechanism, and v2.1.9 did not supply one.** A signed `(version, hash)` with a monotonic revision number stops the *list server* varying bytes per user, but not the *catalog* itself: a catalog can serve client A revisions 1, 2, 3a and client B 1, 2, 3b — each signed, each immutable, each monotonic per client — a **split view**. Two things close it, and both are now requirements rather than the word "transparency" in a table row: (i) **catalog identity is restricted** — the package may pin only a catalog the UA/store trusts as an anchor; if a publisher could pin a publisher-run catalog, model C would collapse into model B and §11's "no server can choose different filtering for you" line would be false. The cost is real and named: every regional or custom list needs a trusted catalog to carry it, which pushes publishers who cannot get one toward `egress + dynamic_rules` — the loud pair, honestly labelled — or package updates. (ii) **Revisions live in an append-only transparency log**: the browser accepts a revision only with an inclusion proof and a consistency proof against the last tree head it saw (or a threshold of independent witness co-signatures on that head), so a catalog that forks its view is detectable by any client and any auditor. With (i)+(ii) the guarantee is earned: *the same bytes for everyone, or evidence of the fork.* The catalog is also an explicit **`catalog_revision` command source** (§5) into `filter_policy` — it is a trusted anchor, and naming it is what lets catalog compromise be a stated residual (global, non-targeted filter control for the duration of a compromised revision; O9) instead of an invisible assumption. Two further consequences the reviews forced:
...
uBlock-class blocker wants to:
    ...
    ✓ Apply filter rules you add yourself (custom filters, the
      element picker, per-site on/off)
...
The blocker's two `!` lines are the honest cost of its in-page scriptlets and its element picker. The scriptlet line is loud pending the per-operator non-interference proofs of O3 (§8); once the browser library discharges them it relaxes to a standard-tier `✓` line ("Disable ad and anti-adblock scripts inside pages"), and the blocker is back to a single `!`. Showing it loud now is the conservative, honest state, not a permanent cost. The "rules you add yourself" line is `filtering.dynamic_rules` (standard; `sink: probe + own_hosts`, §5) — the blocker needs it for custom filters, the picker's output, and the per-site switch, and earlier drafts of both this dialog and the Appendix C manifest omitted it; with the withdrawn `implicit_history` source label it derives nothing on its own, and against the `active_tab` source it adds nothing the `!` line does not already say. The password manager has no `!`, because browser-mediated fill and detection (§19) leave no page-content source and no credential-exfiltration pair. Honest dialog and secure design are the same artifact.

**Marker semantics, pinned.** `✓` lines are *silent*- and *standard*-tier capabilities — silent ones (`filtering.block(subresource)`, `stats.display`) are auto-granted but still *shown*, because "silent" means no consent step, not no disclosure; standard ones are shown and consented to as part of the install. `!` lines are *loud*-tier grants and **any capability, of whatever tier, that carries a derived Axis-1/Axis-2 product by itself** — which is why `content_script.active_tab` (standard, but `page_content × arbitrary_network` in one grant) renders as `!`: the marker follows derived authority, not the tier column. `✗` lines are negative claims the closure proves over the whole granted set. Gated capabilities never appear in the install dialog; they are enabled in settings.
...
**Durable state and the wake contract.** "Durable state survives restarts" is a runtime guarantee with a defined shape, not a hope — without one it is only `chrome.storage.session` renamed. The worker is disposable, but two things it holds do not die with it, because they never lived in the worker process: its **granted authority** (the broker's grant table, §13) and its **declared durable state** (a broker-owned store). Suspending the worker discards only the live JS heap. Two kinds of state, named, because the whole developer cost lives in the gap between them:

```text
durable    data the extension declares it wants kept. Broker-owned,
           disk-backed, survives worker suspend AND full browser
           restart. REHYDRATED before the first event handler runs on
           wake, so the handler sees it synchronously — no async load in
           the hot path, no read-then-decide round-trip.
ephemeral  the live heap — initialized clients, compiled structures,
           parsed config, caches. NOT preserved; rebuilt on cold start.
```

The mechanism is a **broker-journaled store**, not an explicit write call per variable: the worker mutates its durable object and the broker persists the change. Writes are per-key atomic and last-write-wins; the journal is flushed by the point the worker is reported idle, so a crash loses at most the changes since the last event turn, never a torn value (an extension needing a tighter bound may request an explicit flush). Durable state is disk-backed in regular windows and **memory-only in private windows, ending with the session** — this is exactly what `isolated`'s "memory-only storage" means below; durability is a regular-mode property the private instance deliberately forgoes.

The **wake contract** is event-driven and drop-free:

```text
wake source  a declared event only — a subscribed filter/lifecycle
             event, a message to the worker, a scheduled alarm
             (scheduling, §19). No polling, no keepalive; the worker
             cannot and need not keep itself alive.
ordering     on wake: durable state rehydrated -> triggering event
             delivered -> idle timer starts. Events arriving during a
             cold start are QUEUED by the broker and delivered in order
             once rehydration completes, never dropped.
cold start   the engine holds the triggering event's context across the
             wake, so a slow cold start delays handling but never loses
             the event; a cold-start budget is stated (§2-class) and
             overruns are reported, not silently hung.
```

Why this is not Chrome-with-a-nicer-name: the heavy state an MV3 worker rebuilds every wake — a compiled filter set, a parsed rule list — **does not live in the worker here at all** (§1); it is compiled once into the native artifact the engine holds and matches against, so the worker wakes with nothing to recompile. What genuinely must be derived on cold start the extension either recomputes (bounded cheap by design, because §1 leaves little) or **promotes into durable state** — compute once, store the result — turning an expensive initialization into a one-time cost rather than a per-wake one. The honest limit, stated plainly: **the live heap is never restored.** The claim is only that (a) little needs restoring because the engine holds the heavy state, and (b) what remains is cheap or promotable — not that a suspended worker resumes where it left off. A model that promised heap resumption would be lying: no runtime snapshots an arbitrary JS heap across suspend.

Security follows the rest of the model. Durable state is **capability-gated** (`storage.private`, §19) and counts toward the per-extension resident budget (§2), so "keep everything durable" is bounded, not free. It is **broker-owned**: a compromised worker (§16 assumes exactly that) can corrupt only its own store, never widen authority — grants are a separate broker table (§13), and rehydrating hostile durable state re-enters the worker's own sandbox, not the broker's, so the worst a poisoned store does is misbehave within the grants the extension already holds. And it is **partitioned per private-browsing instance** (§12 matrix), never a shared store under a guessable private-prefixed key.
...
extension.private_browsing:
    ...
    isolated_network  isolated, and NO SINK in private — for tools
                      that can work offline; closes the "both instances
                      phone the same host, vendor correlates by IP+time"
                      channel. "No egress" cannot be stated as "no
                      extension-principal network": §15's initiator rule
                      leaves a page's OWN requests as page traffic, so
                      any capability that can plant bytes where page
                      code or the browser later carries them out —
                      a DOM write plus a synthetic click on an existing
                      link, a cookie the browser emits on page traffic,
                      a dynamic rule a publisher server observes — would
                      launder around it. v2.1.9 listed three such
                      capabilities in prose (content_script main-world
                      writes, scriptlet, styles.inject_raw); that was the
                      open-ended denylist the model rejects elsewhere,
                      and it missed cookies.write, dynamic_rules,
                      tabs.open/navigate, downloads.create,
                      omnibox.navigate — and content_script as a whole,
                      which §5 labels sink: arbitrary_network BY
                      DEFINITION. The rule is therefore DERIVED FROM THE
                      REGISTRY, not listed: under isolated_network the
                      private instance is denied every capability whose
                      sink label is not `none`, and every actuator that
                      is page-observable (dom, navigation, session_state,
                      filter_policy via dynamic_rules) — computed from
                      the §19 fields at grant time, so a capability added
                      later is covered without anyone remembering to
                      list it. What remains grantable in private under
                      isolated_network is exactly the sink-free,
                      page-inert subset (static filtering, cosmetic,
                      stats.display, storage.private, ui.*)
...
Any expansion — including removal of a narrowing `gosub` key with permissions unchanged — requires approval before the new version runs. **Pending approval, the previous version keeps running under the previous grant**; it is not suspended. Suspending a blocker until the user notices a consent prompt leaves the user unfiltered — the fail-open outcome §9 refuses for stale lists, and a publisher who wants to punish users into clicking "approve" would get exactly that lever. Only an extension with no prior approved version (a first install) has nothing to fall back to. The composition closure (§5) is recomputed over the new set and the derived-warning **delta** is shown, so a new sink against an old source prompts re-consent. Reductions apply silently. **A registry or model revision is an update too:** when the UA ships a registry that relabels an already-held capability (v2.1.9 gave `tabs.organize` an actuator; v2.1.10 gives `dynamic_rules` a sink), the closure over every installed set is recomputed and any *newly derived* product is surfaced on the extension's management page and at the next install-dialog-equivalent moment — not silently absorbed into a grant the user consented to under the old algebra. Relabeling never *widens* enforcement; it only changes what is disclosed.
...
in-flight    every privileged operation is tagged at validation
             ...
             the wire, a committed navigation, a created file — are
             irreversible; revocation stops the next effect, it cannot
             unsend the last one. A SET COOKIE is the exception: it is
             browser-held state, not wire history, so cookies written
             under cookies.write are PROVENANCE-TAGGED with the writing
             extension and PURGED on revocation of that grant and on
             uninstall — otherwise a session_state sink outlives the
             grant (a tracking cookie planted on publisher.example keeps
             being emitted after the extension is gone)
documents    grant epochs are revalidated on bfcache RESTORE and on
             prerender ACTIVATION, not only on navigation commit — a
             document parked in bfcache across a revocation must not
             resume with the old epoch
...
- **Rotation vs transfer:** ordinary **key rotation preserves the principal** — a new key is the same publisher, and must NOT force re-consent or (worse) *break* a same-publisher pair that legitimately composed. Only a genuine **ownership transfer** (the bound organizational identity changes) changes the principal. **But rotation is authorized by the bound identity, not by the old key.** An earlier draft made "a signed continuity chain from the old key" sufficient — and a *stolen* key can sign exactly that chain, rotating the principal to an attacker key, locking the victim out, and surviving the victim's later revocation of the old key (because the attacker's key is now "up the chain"). So the chain is a *distribution artifact* the store publishes; the *authority* to rotate is the store-bound identity (account authentication, whatever the store's identity proofing is), and a chain the identity did not authorize is treated as a compromise, not a rotation.
...
ownership / identity transfer            principal changes ->
                                         ...
                                         re-consent (new release held until
                                         approved; prior version keeps
                                         running under the prior grant)
key compromise / revocation              old key's packages distrusted;
                                         re-established only up the chain
```

A key rotation that is *not* authorized by the bound identity is treated as a transfer (or a compromise), not a silent same-principal event — so a stolen key can neither quietly ship expanded authority under the victim's identity (the update capability-diff gates every release) nor quietly *become* the identity (rotation needs the account, not the key). Sideloaded extensions with no bound identity are their own singleton principal and never compose with another package. **The same-publisher closure is exactly as sound as the store's identity proofing**: two accounts that are "different organizations" to the store are two principals, and a controller willing to maintain two identities shards the closure. That cost is what the store's proofing sets, and it is named as an O6 dependency rather than claimed away — cross-publisher collusion was already out of manifest-analysis scope; same-publisher closure is the honest-dialog guarantee for publishers who do not go to that length, plus the store-side co-install signals for those who do.
...
**All network egress routes through one policy, keyed by effect not API.** Sonar applies the extension egress policy to *every* network-producing operation attributable to an extension — `fetch`, a DOM-created `<img>`/iframe/form, a navigation, a tab-open, a WebSocket, a beacon, a CSS `url()` — keyed on `(extension_id, initiator, destination)`. This is why the capability is `network.egress_*`, not `network.fetch_*`: `fetch` is one transport among many, and a model that policed only `fetch` would let content scripts and tab APIs launder exfiltration around it. **One policy, two grant families.** "One policy" means one chokepoint and one `(extension_id, initiator, destination)` key — not that every operation consumes the `network.egress_*` host scope. A *transport* the extension reads from (`fetch`, WebSocket, …) is authorized against `network.egress_*(hosts)`. A *navigation* (`tabs.open`/`tabs.navigate`/`omnibox.navigate`, a top-level navigation a content script triggers) is authorized against the navigation capability's own scope — the enumerated `http`/`https` scheme allowlist, any host — and is labelled `sink: arbitrary_network` *on that capability* for exactly that reason (§19). v2.1.9 said both "navigation routes through the egress policy" and "`tabs.navigate` is itself an `arbitrary_network` sink", which read as requiring a tab manager to hold `network.egress_public(<all>)` to restore the user's tabs; it does not — it holds a navigation sink, which the closure accounts for. The address-space check (§15 below) applies to both families alike; the host-*scope* check applies to transports. **The transport set is closed by effect, not by an HTTP-centric list:** WebRTC (ICE candidate gathering and data channels), WebTransport/QUIC, WebSocket, and any raw/UDP socket path an extension can reach are egress and route through the same `(extension_id, initiator, destination)` policy — an extension that cannot satisfy egress policy for a peer/relay simply cannot open the channel. A transport the policy cannot mediate is denied to extensions, never silently exempt, because a P2P/UDP side-channel that skipped the HTTP chokepoint would be exactly the launder the effect-keying exists to prevent.
...
**Identity is channel-bound, and endpoints carry a trust level.** An extension never names itself; the broker creates each IPC endpoint and permanently binds it to an extension identity, so authority derives from the connection. The binding also records *which context* holds the endpoint, because the threat model differs: the **worker** endpoint is compromised only if the extension is; a **content-script** endpoint lives inside a page renderer, and a hostile page that exploits that renderer *is* every content script injected into it, with whatever those endpoints can invoke. So content-script endpoints are **renderer-trust**: they may invoke only document-scoped operations on their own document (DOM access they already have, messaging to their own worker, the §13-bound per-document grants); every other capability — `network.egress_*`, `cookies.*`, `tabs.*`, `downloads.*`, `storage.*` writes, `dynamic_rules`, anything OS-touching — is callable only from the worker endpoint, and a content script that wants them asks its worker. A content-script `fetch` is still egress attributed to the extension principal (§15) and still bounded by the extension's egress scope, but the point of the split is that a page exploit gains at most *that* scope for *that* document, never the worker's full authority. Without the split, "the broker assumes a compromised worker" would be the wrong threat model: the cheapest compromise is the renderer. Capability references are unforgeable — **kernel-mediated (SCM_RIGHTS) by preference**, since a passed file descriptor cannot be forged or replayed across connections; where a platform lacks it, a per-connection random handle wide enough to resist brute force (≥128-bit) and bound to that connection's identity (never replayable onto another) is the fallback, never an integer index the worker supplies. Renderer-side identity (frame, document, origin) comes from the process topology, never from message fields. This closes the confused-deputy and capability-forgery classes.
...
$3p / $third-party / $1p,
$domain= / $from= / $to=,
DNR initiatorDomains /
  domainType, on ANY rule    -> kept at the rule's own tier where the
                                predicate is LEAK-FREE (§5): party and
                                destination are always leak-free; an
                                initiator/top-frame predicate is leak-
                                free for top-level and same-site
                                subresource requests (the destination
                                sees Referer/Origin/Sec-Fetch-Site) and
                                LEAKING for requests issued from a
                                nested cross-origin frame or under a
                                referrer-suppressing policy — the
                                compiler splits such rules: the leak-
                                free projection compiles as-is, the
                                leaking residue needs the loud source
                                grant (network.observe-class) or is
                                dropped and reported
$csp= / $permissions=        -> dropped: CSP / Permissions-Policy are
                                protected headers (§15). A future
                                ADDITIVE-only CSP capability (policies
                                intersect, so adding one only tightens)
                                would have to strip report-uri /
                                report-to (violation reports are a sink
                                carrying the document URL)
$removeheader=               -> filtering.headers.*.remove under the
                                §15 allowlist + §5 leak-free criterion
$header=                     -> response-header predicate, §15
                                matchable set only
$urlskip=                    -> dropped: navigates to a URL extracted
                                from the page URL — a page-supplied
                                destination chosen by rule; not
                                expressible below filtering.redirect_*
...
## 19. Capability Registry (v0.2.13)
...
filtering.headers.request.remove      standard   modifiable safe-list (§15
filtering.headers.request.set_safe    standard   admission criterion); values
filtering.headers.response.remove_safe standard  RFC-9110 validated; standard
filtering.headers.response.set_safe   standard   ONLY for rules meeting the §5
                                      leak-free criterion (predicate over
                                      facts the destination already holds;
                                      installed by package/catalog/user) —
                                      a rule whose predicate leaks (top-
                                      frame from a nested frame, other-
                                      request/page facts) compiles as
                                      source: implicit_history|page_content,
                                      loud; source: none, sink: none for
                                      the leak-free set
  (matchable header set is narrower than modifiable; Authorization/
   Cookie/Set-Cookie are not matchable)
filtering.cosmetic                    silent; source: none (observable
                                      collapse is fine — every extension
                                      context that could observe layout
                                      already holds page_content, §8); no
                                      attacker-chosen URL (§6)
filtering.procedural                  silent; closed DSL, cost-bounded,
                                      no layout-aware ops, no :xpath
filtering.dynamic_rules               standard; sink: probe + own_hosts
                                      (the worker's runtime CHOICE of rule
                                      is an outbound channel to any server
                                      the publisher observes from page
                                      traffic — §5 leak-free clause (2);
                                      derives history/page.exfiltration
                                      against any source held) +
                                      source: none (the v2.1 implicit_
                                      history label is withdrawn — no
                                      mechanism, §5) +
                                      ...
                                      envelope (§14)
filtering.remote_rulesets(catalog)    standard; command-source:
                                      catalog_revision; browser-fetched,
                                      MODEL C (§9): package pins a UA/
                                      store-TRUSTED catalog identity+key
                                      (never a publisher key), catalog
                                      signs (version,hash) revisions in an
                                      APPEND-ONLY transparency log —
                                      accepted only with inclusion +
                                      consistency proofs (or witness
                                      co-signatures) — with a freshness
                                      max-age; reject on hash/
...
filtering.rewrite_url(class, hosts)   standard, ALL classes — no silent
                                      ...
                                      named (§5, §6);
                                      source: none, sink: none FOR RULES
                                      MEETING THE §5 LEAK-FREE CRITERION
                                      (destination/party/URL predicates;
                                      package/catalog/user-installed); a
                                      rule predicated on a fact the
                                      destination lacks compiles as a
                                      source (loud), same as headers
...
network.auth_provider                 loud       source: implicit_history
                                      (which origins issue challenges) +
                                      sink: arbitrary_network. The sink is the
                                      non-obvious half: the extension CHOOSES
                                      the credential bytes sent to any origin
                                      that challenges, so the username field is
                                      an outbound channel to a server the
                                      publisher can cause to challenge. Answering
                                      a challenge is also authenticating AS the
                                      user where the extension holds the secret
...
content_script(hosts)                 loud   source: page_content +
                                             ...
                                             command-source: webpage (the
                                             page can drive it; where `hosts`
                                             contains a publisher-controllable
                                             origin this is also a remote_
                                             server channel, §5) +
                                             actuator: dom + navigation
                                             (inherent — shares the DOM,
                                             can navigate); its IPC endpoint
                                             is renderer-trust (§16)
content_script.active_tab             standard  same labels; browser-rendered
                                             gesture only; frame-tree/epoch
                                             bound; renders as `!` (self-
                                             derived page.exfiltration, §11)
page.main_world_inject(hosts)         loud (or gated); command-source:
                                             webpage; actuator: dom +
                                             navigation; bidirectional trust,
                                             hardening review of injected surface
content.declarative_show              silent   source: none; sink: none;
                                             actuator: browser_ui. The pattern
                                             the model asks for, so it is priced
                                             accordingly: the BROWSER matches
                                             page content against declared
                                             predicates and toggles the
                                             extension's own action. No host
                                             permission, no page access, and —
                                             load-bearing for `source: none` —
                                             no callback, so a rule firing tells
                                             the extension nothing. If a future
                                             variant reports matches back, that
                                             variant is page_content and loud
...
forms.fill_auth(origins)              standard  source: none; sink: none;
                                             command-source: none;
                                             actuator: session_state. The
                                             `forms.fill` pattern applied to the
                                             BROWSER's HTTP auth prompt
                                             (401/407) rather than to a page
                                             form, and standard for the same two
                                             reasons: each fill needs a FRESH
                                             browser-rendered gesture the
                                             extension cannot synthesize, and
                                             the extension cannot CHOOSE the
                                             credential — the browser selects
                                             from the origin-bound store and the
                                             user confirms. The actuator is
                                             session_state because completing a
                                             challenge is what establishes the
                                             authenticated session.
                                             What it deliberately does NOT do is
                                             the whole point, since it exists to
                                             replace `network.auth_provider`:
                                             the extension is never told a
                                             challenge occurred, so there is no
                                             `implicit_history` source (which
                                             origins challenge is a browsing
                                             fact), and it never supplies
                                             credential BYTES, so there is no
                                             `arbitrary_network` sink (the
                                             username field of a challenge the
                                             publisher can provoke is an
                                             outbound channel). REQUIREMENT, not
                                             a consequence: no per-challenge
                                             feedback of any kind reaches the
                                             extension — not success, not
                                             failure, not a count. A build that
                                             reports fills back for accounting
                                             or UI has re-created the source and
                                             is loud
...
capture.page_archive                  loud   source: page_content; saves the
                                      page AND its subresources (MHTML) — the
                                      bytes, not a picture of them
capture.desktop                       gated  source: pixels; persistent
                                      indicator. Strictly broader than
                                      capture.tab_*: it leaves the browser
                                      entirely and captures other applications,
                                      so it is not a page capability at all
...
tabs.restore(handle)                  standard  browser-managed TAB/SESSION
                                             STORE: the browser records URLs
                                             it closes/suspends under opaque
                                             handles; the extension navigates
                                             by HANDLE, never by URL bytes, on
                                             a browser-rendered gesture —
                                             sink: none, actuator: navigation
                                             (same pattern as forms.fill: the
                                             browser chooses the bytes). This
                                             is what keeps a tab manager out of
                                             the history.exfiltration pair
                                             (tab_urls × navigate) — Appendix A
tabs.open / tabs.navigate             standard  sink: arbitrary_network +
                                             actuator: navigation; authorized
                                             against THIS capability's scope
                                             (any http/https host), not the
                                             network.egress_* host scope (§15);
                                             SCHEME
...
storage.sync                          standard sink: own_hosts-class (the UA's
                                             sync service) + a cross-DEVICE
                                             channel between instances of the
                                             same extension; denied/partitioned
                                             in private (§12)

-- Messaging (command channels, §5) --
messaging.external_web(origins)       standard command-source: webpage +
                                             actuator: extension_bridge
                                             (externally_connectable with web
                                             origins / onMessageExternal): the
                                             named origins can DRIVE the
                                             extension. Derived: webpage × os
                                             -> page.os_control when held with
                                             native_messaging (the web →
                                             extension → native-host chain);
                                             webpage × navigation/dom/browser_ui
                                             likewise. Was entirely absent from
                                             the registry before v0.2.10
messaging.push                        standard command-source: remote_server
                                             WITHOUT any egress grant (a push
                                             service delivers publisher bytes);
                                             composes with every actuator held
messaging.external_extension(ids)     standard mutual externally_connectable;
                                             forms a communicating set (§5);
                                             actuator: extension_bridge

-- Clipboard & installed-extension state --
clipboard.write                       standard actuator: os (a planted string
                                             is a user-intent power — a swapped
                                             payment address — and remotely
                                             drivable: remote_server × os)
clipboard.read                        loud     source: user_text + credentials
                                             (clipboards carry passwords);
                                             gesture-scoped only
extensions.list                       gated    source: installed_extensions —
                                             a co-install ORACLE that lets a
                                             publisher detect its own pair /
                                             a competitor; not default for
                                             ordinary extensions
...
bookmarks.read                        loud      source: implicit_history;
                                      sink: none; command-source: none;
                                      actuator: none. URLs the user chose to
                                      keep — a CURATED history, and a sharper
                                      profile than the visited set because the
                                      selection itself is the signal. Typed
                                      with the history atom because the product
                                      is the same (history.exfiltration); the
                                      dialog wording stays specific through the
                                      capability NAME, which is what §11 renders
bookmarks.write                       standard  source: none;
                                      sink: own_hosts (the UA's sync service);
                                      command-source: none;
                                      actuator: browser_ui. Two properties the
                                      "it only edits bookmarks" reading misses.
                                      (1) Bookmarks SYNC: a written bookmark is
                                      attacker-chosen bytes that leave the
                                      device and reappear on every other
                                      instance signed into the account — the
                                      same cross-device channel storage.sync
                                      carries, so it is a sink and §12's
                                      label-derived denial set covers it in
                                      private browsing without naming it. (2) A
                                      planted bookmark is a NAVIGATION vector
                                      the user later clicks, under a title the
                                      extension chose; it is not authority over
                                      navigation (the user still acts), which is
                                      why the actuator is browser_ui rather than
                                      navigation
history.read                          loud      source: implicit_history;
                                      sink: none; command-source: none;
                                      actuator: none. The literal visit store —
                                      the strongest instance of the atom, and
                                      strictly more than tab_urls: getVisits
                                      returns WHEN as well as what, so the
                                      record is a timeline, not a set
history.write                         standard  source: none;
                                      sink: own_hosts (the UA's sync service);
                                      command-source: none;
                                      actuator: browser_ui. Covers both
                                      directions Chrome bundles, and neither is
                                      inert: `addUrl` FABRICATES visits the user
                                      never made (integrity — the history is
                                      evidence, and it syncs, so the fabrication
                                      propagates), while `deleteUrl`/`deleteAll`
                                      DESTROY user data irreversibly and are
                                      anti-forensic — an extension can erase the
                                      record of where it sent the user. Standard
                                      only because neither reads anything; held
                                      with history.read the pair is a full
                                      read-write grant over the timeline

favicon.read                          loud      source: implicit_history;
                                      sink/command-source/actuator: none.
                                      Not "an icon": asking for the favicon of
                                      an arbitrary URL is a VISITED-URL ORACLE
                                      — the answer differs for a page the user
                                      has been to. The capability is the query,
                                      not the image
top_sites.read                        loud      source: implicit_history. The
                                      most-visited list names real URLs, so it
                                      is history in projection, not `aggregate`
                                      (which §5 reserves for dimensionless
                                      counts). A short list is a sharper
                                      profile than a long one
reading_list.read                     loud      source: implicit_history
reading_list.write                    standard  sink: own_hosts (sync);
                                      actuator: browser_ui. Same synced-store
                                      shape as bookmarks.write
browsing_data.clear                   loud      source: none; sink: none;
                                      actuator: session_state + browser_ui.
                                      I:high, A:high. Loud DESPITE reading
                                      nothing, because the sentence informed
                                      consent needs is "can delete your browsing
                                      data, including saved passwords, and sign
                                      you out everywhere" — `removalOptions`
                                      reaches passwords and formData, not just
                                      history. Destruction is also
                                      anti-forensic: it erases the record of
                                      what the extension itself did. This is the
                                      one entry in v0.2.12 whose tier is a
                                      judgement rather than a reading; it is
                                      louder than history.write because that one
                                      reaches only the timeline

-- Identity & authorization --
identity.web_auth_flow(origins)       standard  source: credentials;
                                      command-source: user; sink: none;
                                      actuator: none. The BROWSER opens and
                                      owns the auth window at a declared
                                      provider origin; the extension cannot
                                      read it, so what the user types never
                                      reaches the extension — it receives only
                                      the final redirect. Standard for the same
                                      reason forms.fill is: the mediating step
                                      is browser-owned and the consent screen
                                      is the PROVIDER's, naming its own scopes.
                                      Origins are pinned at install (§18 never
                                      translates to a wildcard); the returned
                                      redirect is still `credentials`, so with
                                      any sink the closure derives
                                      credential.exfiltration
identity.auth_token(scopes)           loud      source: credentials; sink: none;
                                      command-source: none; actuator: none.
                                      A bearer token for the BROWSER PROFILE's
                                      signed-in account. Distinct from
                                      web_auth_flow in the property that
                                      matters: after first consent the token is
                                      re-minted SILENTLY — no window, no fresh
                                      gesture — so this is ambient authority
                                      over the user's account for as long as
                                      the grant is held, which is what earns
                                      loud. Scopes are pinned from the manifest
                                      at install; widening them in an update is
                                      an expansion and triggers §13 re-consent.
                                      The token confers provider-side authority
                                      the browser cannot narrow further, so the
                                      grant is revocable only wholesale
identity.profile_info                 loud      source: account_identity;
                                      sink: none; command-source: none;
                                      actuator: none. The signed-in account's
                                      email + stable account id — a cross-site
                                      identifier for the HUMAN, not a page fact
                                      and not a secret, which is why §5 types it
                                      with its own atom rather than
                                      `credentials`. Its danger is JOIN, not
                                      confidentiality: held with any other
                                      source it makes that source attributable
                                      to a named person, so the closure derives
                                      identity.exfiltration against any sink.
                                      Chrome gates this behind a separate
                                      `identity.email` permission and names it
                                      in its own dialog line; the same
                                      separation holds here
...
devtools.dom                          loud      source: page_content;
                                      actuator: dom
ui.side_panel                         standard  actuator: browser_ui;
                                      source: none; sink: none;
                                      command-source: user. A PERSISTENT
                                      extension-owned frame docked ALONGSIDE
                                      page content — never overlaying it, so it
                                      cannot be used for UI redress against the
                                      page it sits next to. Standard rather
                                      than silent (unlike ui.toolbar) because
                                      persistent screen real estate adjacent to
                                      the page is a spoofing surface: it needs
                                      the same browser attribution + restore
                                      ui.newtab_override carries, showing which
                                      extension owns the panel. Its document is
                                      the EXTENSION's own origin and isolated
                                      principal — it is not page access, and a
                                      panel that wants page content needs
                                      content_script like anything else.
                                      Programmatic open requires a browser-
                                      rendered user gesture (the extension
                                      cannot force the panel open), and per-tab
                                      panel options are keyed by opaque tab
                                      HANDLE, never by URL — the same rule that
                                      keeps tabs.restore out of tab_urls, so
                                      the panel API never becomes a source.
                                      Derived: remote_server × browser_ui ->
                                      remote.ui_control, as for every browser_ui
                                      actuator
ui.newtab_override                    standard  actuator: browser_ui; browser
                                      attribution + restore

-- Browser settings --
browser_settings.privacy              gated  source: none; actuator: browser_ui.
                                      Gated because these toggles WEAKEN the
                                      browser rather than configure it: the same
                                      API disables Safe Browsing, changes WebRTC
                                      IP handling (exposing local addresses),
                                      and turns off third-party cookie blocking.
                                      An extension that can lower the browser's
                                      own defences is a settings-level decision,
                                      not a dialog line
browser_settings.content              gated  source: none; actuator: browser_ui.
                                      Per-ORIGIN permission state — JavaScript,
                                      popups, cookies, and the device
                                      permissions the web model otherwise
                                      mediates. It can grant a site something
                                      the USER previously denied, which is
                                      authority over the permission system
                                      itself
browser_settings.accessibility.write  standard actuator: browser_ui. Toggling
                                      spoken feedback/magnification is user-
                                      visible and reversible
browser_settings.accessibility.read   loud     source: device_fingerprint.
                                      Loud where the other fingerprint readers
                                      are standard, because the entropy is not
                                      the whole story: "a screen reader is in
                                      use" is a health-adjacent inference about
                                      the person, not a property of the machine.
                                      §5 has no atom for sensitive-category
                                      inference, so the atom carries the
                                      linkability and the TIER carries the rest —
                                      stated here rather than left implicit
browser_settings.fonts.read           loud     source: device_fingerprint.
                                      `getFontList` ENUMERATES the fonts
                                      installed on the machine, which is the
                                      classic high-entropy fingerprinting vector
                                      and the one member of this group strong
                                      enough to identify on its own
browser_settings.fonts.write          standard actuator: browser_ui
downloads.hide_ui                     standard actuator: browser_ui. Alone it is
                                      a preference; with downloads.create it is
                                      the concealment half of a silent download,
                                      which is why it is named separately rather
                                      than folded into downloads.control

-- System & lifecycle --
system.display_info                   standard source: device_fingerprint;
                                             geometry, DPI and multi-monitor
                                             layout — the highest-entropy member
                                             of this group
system.cpu_info                       standard source: device_fingerprint
system.memory_info                    standard source: device_fingerprint
system.storage_info                   standard source: device_fingerprint
...
Still open:
  ...
  O2  Exhaustive WebExtension API-surface list and priority. v2.1.10
      pulled forward the entries that introduce new atoms or high-
      impact actuators (messaging.external_web, messaging.push,
      storage.sync, clipboard.*, extensions.list, tabs.restore); still
      unregistered and known to matter: identity.launchWebAuthFlow
      (navigation + read-back), history/webNavigation/sessions/topSites
      (tab_urls sources), bookmarks, browsingData (session_state
      actuator), contentSettings/privacy (filter_policy-class
      actuators), debugger (devtools.inspected_eval-class), printing,
      webRequestAuthProvider (credentials).
  ...
  O5  stats privacy-budget parameters (criterion: a single-site probe
      ...
      parallelize past the bound; ALSO rate-limited per extension so
      one extension cannot drain the pool in a burst (pool DoS);
      cross-PROFILE parallelization (N profiles = N pools) is a named
      residual, since per-device pooling needs OS-level trust the model
      does not assume.
  O6  Signing / trust MECHANISM (the UA-side implementation of §17
      ...
      publisher-principal closure (§5) depends on them. Two dependencies
      the model states but cannot discharge: the STORE'S IDENTITY
      PROOFING (the same-publisher closure is exactly as sound as the
      store's ability to tell one controller with two accounts from two
      controllers — §13), and rotation being authorized by that bound
      identity rather than by a key-signed chain (§13). The catalog
      transparency log / witness set for model C (§9) is the same
      trust-mechanism family.
  O7  Covert-channel review (storage/cache/DNS/timing/quota) — the one
      ...
      structural controls — rewrite_url's destination-server
      observability (§6) — and the POOLED STATS BUDGET as a low-
      bandwidth browser-provided channel between unrelated publishers
      (A depletes, B observes; §7).
  ...
  O9  Named residuals the model does not close, only bounds: the socket
      ...
      that bounded window); CATALOG COMPROMISE more generally (a trusted
      anchor, `catalog_revision` command source — a bad revision is
      GLOBAL, non-targeted filter control until detected through the
      transparency log; model C trades targeted-policy risk for this
      centralized one, §9); and the revocation COMMIT residual (bytes
      already on the wire cannot be unsent — pre-emption bounds new
      effects, not committed ones; extension-written cookies are the
      reversible exception and are purged, §13).
```
...
## Changelog: v2.1.9 → v2.1.10

One pass (Claude) against v2.1.9. It re-examined the v2.1.9 headline rule and found it right in direction but mis-scoped and under-applied; the rest are the model's own principles applied where the registry, translation table, and worked examples had not caught up, plus four fresh model-level gaps (`dynamic_rules` as outbound sink, the egress-centric `remote_server`, model C's unproven "same bytes" claim, the isolated-realm surrogate contradiction).

1. **Leak-free criterion replaces "unconditional" (§5, §15, §18, §19).** A mutation is `source: none` iff (1) its predicate uses only facts the observer already holds and (2) it was installed by package/catalog/user, not by the worker at runtime. This un-bans `$3p`/`$domain=`/DNR `initiatorDomains` where the destination already sees the fact (`Sec-Fetch-Site`, `Referer`), names the genuinely leaking predicates (top-frame identity from nested frames, referrer-suppressed initiators, other-request/page facts, and the request URL itself when a *third* server observes the effect), extends the rule to `block`/`allow`/`upgrade_scheme` (a conditional exception is a presence channel), and is carried into §19's header/rewrite entries and §18's modifier translation. *(Claude #2)*
2. **`filtering.dynamic_rules` is an outbound sink (§5, §19).** Clause (2) of the criterion: the worker's runtime *choice* of rule encodes its state into what publisher-observable servers see (header variant *k*, block/unblock a beacon) — `sink: own_hosts`, with no egress grant. The v2.1 `source: implicit_history` label is withdrawn (no mechanism was ever described; it only produced a false derived warning on the flagship). *(Claude #1, #7)*
3. **`remote_server` = any channel delivering remotely chosen bytes to extension code (§5, §19).** Not just readable egress: web-page→extension messaging (new `messaging.external_web` — previously absent, the web→extension→native chain), push (new `messaging.push`), content scripts on publisher-controllable origins, remote iframes in extension pages; `content_script`/`page.main_world_inject` carry `command-source: webpage`. *(Claude #4)*
4. **Model C earns "same bytes for everyone" (§9, O9).** Catalogs are UA/store-trusted anchors only (a publisher key is model B); revisions live in an append-only transparency log accepted with inclusion + consistency proofs or witness co-signatures (closes split view); the catalog is an explicit `catalog_revision` command source and its compromise a named global residual. *(Claude #3)*
5. **Axis 2 closed (§5).** A product for every command-source × actuator pair, including `remote_server × os/session_state/extension_bridge`, `webpage × *`, `native_process × *`, `enterprise_policy × *`, `publisher_update × *`, `catalog_revision × filter_policy`; dead atoms now carried by registry entries. *(Claude #9)*
6. **`isolated_network` denial set is label-derived (§12).** Every `sink ≠ none` and every page-observable actuator is denied in private — computed from §19, not listed; covers `cookies.write`, `dynamic_rules`, `tabs.navigate`, `downloads.create`, and `content_script` wholesale. *(Claude #5)*
7. **Navigation separated from the egress host scope; `tabs.restore` added (§15, §19, Appendix A).** `tabs.open/navigate` are authorized against their own http/https scope and are sinks on their own; a browser-managed opaque tab/session store (`tabs.restore(handle)`) keeps tab managers out of `tab_urls × navigate`. *(Claude #6)*
8. **Header allowlist admission criterion (§15).** No UA-side fetch/navigate/store/trust/MIME/security semantics — a static `Link: rel=preload` is history exfiltration to a third server; `Content-Type` is XSS. *(Claude #8)*
9. **Key rotation authorized by the bound identity, not a chain (§13).** A stolen key can sign a continuity chain; it cannot authenticate as the store account. Store identity proofing named as the O6 dependency of the same-publisher closure. *(Claude #10, #23)*
10. **API-stub surrogates are main-world/scriptlet-class (§6, §8).** An isolated realm cannot define `window.googletag`; `gpt.js`/`ga.js`-class surrogates join the §8 library under its closure and tier; the isolated realm is for passive stubs. Redirects are in-place response substitutions (`response.url` unchanged) so per-session WAR randomization is not defeated by `fetch().url`. *(Claude #11, #15)*
11. **Filtering scope is `(initiator_scope, destination_scope)` (§14).** *(Claude #12)*
12. **Content-script IPC endpoints are renderer-trust (§16).** Document-scoped ops only; egress/cookies/tabs/downloads/dynamic_rules from the worker endpoint. *(Claude #13)*
13. **Expansion runs the prior version pending consent (§13)** — suspension fails open by §9's own argument. Registry relabels recompute and re-surface derived deltas for installed sets. *(Claude #14, #22)*
14. **Cosmetic compositor-invisibility dropped (§8, §19)** — impossible with collapse semantics and unnecessary (any extension observer already holds `page_content`). *(Claude #17)*
15. **Appendix D sanity-checked against the `AdGuard/Scriptlets` catalogue.** The write-only/reads-page-data partition holds across ~100 real operators (the read-based arsenal — abort-*/prevent-*/*-prune/*-response — is the loud majority, as predicted). Refinements folded in: the standard-eligible class is a recognizable neutralizer family (adds `nowebrtc`, plain `noeval`, fixed-enum storage setters/removers, `window.name` defusers), `set-attr` tightened to fixed-enum value (an arbitrary static string is a destination gadget), and cookies (`session_state` sink, loud) distinguished from `localStorage` (integrity-only, standard-eligible). Untrusted `set-constant`'s real restricted value-set confirms G2; `trusted-*` is exactly the excluded arbitrary-value family.
16. **Smaller:** pooled stats budget named as a cross-publisher channel + per-extension depletion rate limit (§7, O5, O7); extension-written cookies provenance-tagged and purged on revoke/uninstall; bfcache/prerender epoch revalidation (§13); host-scope check precedes DNS resolution (§15); adblock-rust is a verdict oracle, not an effect path (§14); `$csp`/`$permissions`/`$removeheader`/`$header`/`$urlskip` translation pinned (§18); `storage.sync`, `clipboard.*`, `extensions.list` registered, remaining O2 items enumerated; dialog marker semantics pinned and stale lines fixed (§11, Appendix C). *(Claude #16, #18–#21, #24)*

---

## Changelog: v2.1.10 → v2.1.11

No red-team pass. This version comes from **measurement**: `manifest::translate`
was run over 10,165 live Chrome Web Store manifests and asked, for every
permission, which §19 capability covers it. Permissions with no entry were
reported rather than ignored, and the resulting list — ranked by how many real
extensions hold each — is the first evidence-driven view of what the registry
was missing. Two gaps dominated it.

1. **`ui.side_panel` added (§19).** 673 of 10,165 extensions request
   `sidePanel` and the registry had no entry, so the translation produced no
   grant for a *persistent extension-owned frame docked beside page content*.
   Standard rather than silent (unlike `ui.toolbar`): persistent screen real
   estate adjacent to the page is a spoofing surface and needs the browser
   attribution + restore `ui.newtab_override` already carries. The entry pins
   the three properties that keep it out of the source lattice — adjacent and
   never overlaying (no UI redress against the neighbouring page), the
   extension's own isolated principal (a panel wanting page content needs
   `content_script` like anything else), and per-tab options keyed by opaque tab
   handle rather than URL, the same rule that keeps `tabs.restore` out of
   `tab_urls`. Programmatic open requires a browser-rendered gesture.

2. **`identity` split three ways (§19).** 432 extensions request it. Chrome's
   one permission covers three different authorities and the split follows the
   `downloads`/`tabs` precedent: `identity.web_auth_flow(origins)` is standard
   because the *browser* owns the auth window — what the user types never
   reaches the extension, and the consent screen is the provider's, the same
   reasoning that makes `forms.fill` standard; `identity.auth_token(scopes)` is
   loud because after first consent the token is re-minted **silently**, with no
   window and no fresh gesture, which is ambient authority over the user's
   account for as long as the grant is held; `identity.profile_info` is loud and
   separately gated, as Chrome separates `identity.email`.

3. **`account_identity` source atom added (§5).** Typing `identity.profile_info`
   exposed a hole in the closed vocabulary: an email address and a stable
   account id are neither `credentials` (they authenticate nothing) nor any
   other atom. Stretching `credentials` to cover them would over-warn on the
   confidentiality axis while *under*-stating the real property. The danger of
   an account identifier is **join, not secrecy** — it makes every other source
   in the set attributable to a named person. `tab_urls` alone is a browsing
   history; `tab_urls + account_identity` is *this person's* browsing history.
   Axis 1 gains `account_identity × sink -> identity.exfiltration`.

4. **Measurement notes carried back into the translation table (§18).** Host
   patterns are scopes *wherever the dialect puts them* — MV2 keeps them inside
   `permissions`, and reading them only from `host_permissions` silently
   narrowed those grants (a manifest with `scripting` + a host pattern in
   `permissions` produced no `content_script` grant at all, failing closed but
   reporting nothing). Chrome permissions with no registry entry are now a
   distinct reported class from strings Chrome itself discards; conflating the
   two hid this list. An invented `idle` capability that §19 never defined was
   withdrawn — it is a gap, not an entry.

*Still open, ranked by real-world frequency:* `bookmarks` (101), `idle` (70),
`system.display` (62), `history` (60), `favicon` (53), `browsingData` (46),
`webRequestAuthProvider` (35), `topSites` (32), `tts` (24),
`declarativeContent` (23).

**Also in this version (not measurement-driven):** §12 gains a **durable-state and wake contract**, turning "durable state survives restarts, keepalive hacks are pointless" from an assertion into a mechanism — a broker-journaled, disk-backed durable store rehydrated *before* the first event handler runs (memory-only and session-scoped in private, per `isolated`); an event-only, drop-free wake contract (cold-start events queued, never lost); and the honest data-vs-derived-objects limit — the live JS heap is never restored, but the engine holds the heavy state (§1) so little needs rebuilding and what does is cheap or promotable into durable state. Broker-owned and capability-gated (`storage.private`, §16, §2), so a compromised worker corrupts only its own store. (Fills the §12 gap surfaced by the MV3 service-worker DX discussion.)

---

## Changelog: v2.1.11 → v2.1.12

Measurement-driven, like v2.1.11: the next two entries on the ranked gap list
produced by translating 10,165 live Chrome Web Store manifests. No §5 change —
the existing atoms type both correctly.

1. **`bookmarks.read` / `bookmarks.write` added (§19).** 101 of 10,165
   extensions request `bookmarks`. Read is loud: a curated history is a sharper
   profile than the visited set, because the user's selection is itself the
   signal.

2. **`history.read` / `history.write` added (§19).** 60 extensions. Read is
   loud and strictly stronger than `tab_urls` — `getVisits` returns timestamps,
   so the record is a timeline rather than a set.

3. **The write side of both is a SINK, not a local edit.** Bookmarks and
   history both sync: an entry the extension writes is attacker-chosen bytes
   that leave the device and reappear on every instance signed into the
   account. That is the cross-device channel `storage.sync` already carries, so
   both are typed `sink: own_hosts` — which composes on Axis 1 with any source
   the set holds, and pulls both into §12's label-derived private-browsing
   denial set automatically. `history.write` additionally covers fabrication:
   `addUrl` plants visits the user never made, into a store that is treated as
   evidence and that propagates.

4. **Both reads reuse `implicit_history`.** A per-store atom was considered and
   rejected: the Axis-1 product is `history.exfiltration` either way, and §11
   takes its dialog wording from the capability name (`bookmarks.read`), not
   from the atom. Minting `bookmark_urls` would have added a §5 member that
   changes no product and no sentence.

5. **The rest of the ranked list that existing atoms already type (16 more
   entries).** Grouped because they needed care but no new vocabulary. The ones
   worth reading twice: `favicon.read` is a visited-URL ORACLE rather than an
   icon fetch; `network.auth_provider` is an `arbitrary_network` SINK, since the
   extension picks the credential bytes sent to any origin it can cause to
   challenge; `tts.provide_engine` READS everything asked to be spoken, the
   opposite of how the permission name scans; `downloads.hide_ui` is inert alone
   and the concealment half of a silent download when held with
   `downloads.create`; and `content.declarative_show` is priced silent because
   it is exactly the shape the model asks for — the browser matches, and no
   callback means `source: none`.

6. **Two tiers that are judgements, not readings.** `browsing_data.clear` is
   loud despite reading nothing, because `removalOptions` reaches saved
   passwords and session cookies and the consent sentence has to say so; it is
   deliberately louder than `history.write`, which reaches only the timeline.
   `browser_settings.privacy` and `.content` are gated rather than standard
   because they WEAKEN the browser — disabling Safe Browsing, exposing local
   addresses through WebRTC, or granting an origin something the user denied is
   a settings-level decision, not a dialog line.

7. **One residual named rather than typed.** `system.idle_state` is
   `source: none` because §5 has no atom for it, but polled against any sink it
   is a PRESENCE TIMELINE — when the human is at the machine. It is filed with
   the device-fingerprint question below rather than silently labelled.

8. **`device_fingerprint` source atom added (§5), typing the last measurable
   group.** `system.display` (62), `accessibilityFeatures.read` (15),
   `system.memory` (8), `fontSettings` (7), `system.cpu` (3) and
   `system.storage` (2) all leak one thing, and the closed vocabulary had no
   name for it. It is not `account_identity` — there is no account, and it
   identifies the machine rather than the person; it is not `aggregate`, which
   §5 reserves for dimensionless counts. Its danger is LINKABILITY across the
   boundaries the rest of the model maintains: the same fingerprint is readable
   inside and outside private browsing (§12), and survives clearing every cookie
   in the profile. Axis 1 gains
   `device_fingerprint × sink -> fingerprint.exfiltration`.

   The tiers say the rest. The four `system.*_info` readers are standard — no
   one of them is alarming, and the composition is what the closure surfaces.
   `browser_settings.fonts.read` is loud because `getFontList` enumerates
   installed fonts, the one member strong enough to identify alone.
   `browser_settings.accessibility.read` is loud for a different reason:
   "a screen reader is in use" is a health-adjacent inference about the PERSON,
   and since §5 has no atom for sensitive-category inference, the atom carries
   the linkability while the tier carries the rest.

*Still open.* One group. **A scope question:** `printerProvider` (6), `printing` (2),
`printPreview` (2), `enterprise.deviceAttributes` (1),
`webAuthenticationProxy` (1) — whether Gosub implements ChromeOS and enterprise
APIs at all is a product decision, and until it is made, reporting them as
unnamed authority is the correct behaviour rather than a defect.

---

## Changelog: v2.1.12 → v2.1.13

Registry v0.2.13. One new entry; no §5 change.

1. **Appendix C2 added — a password manager, translated.** §11 uses one as its
   model citizen, showing a dialog with no `!` line; the live Bitwarden manifest
   translates to six. Both are correct, and the distance between them is the
   model's own argument, so it is now written down with the substitution table
   that closes it: `forms.detect_credentials` + `forms.fill` +
   `network.egress_public(vault host)` in place of `content_script(<all_urls>)`
   + `tabs` + `webNavigation` + `webRequest`.

2. **One registry gap named by that exercise.** `network.auth_provider`
   (`webRequestAuthProvider`) has no mediated equivalent — there is no
   browser-owned filler for HTTP Basic/Digest prompts the way `forms.fill` is
   browser-owned for page forms. A password manager supporting HTTP auth
   therefore cannot avoid a loud grant that is also an `arbitrary_network` sink.
   Filed as a gap rather than left implicit.

3. **`forms.fill_auth(origins)` added (§19) — the gap C2 exposed, closed in the
   same version that named it.** `network.auth_provider` is an
   `implicit_history` source and an `arbitrary_network` sink, and both labels
   exist only because the extension is in the loop: it is handed the challenge
   event (learning which origins challenge) and it supplies the credential bytes
   (a channel to any origin it can provoke). `forms.fill_auth` applies the
   `forms.fill` pattern to the browser's 401/407 prompt — fresh browser-rendered
   gesture, browser selects from the origin-bound store, user confirms — so both
   labels go to `none` and the tier to standard. `actuator: session_state`,
   because completing a challenge establishes the session. The no-feedback rule
   is written as a REQUIREMENT: any per-challenge report back to the extension
   re-creates the source and is loud.

   Not reachable from a Chrome manifest — no permission maps to it — so it is a
   target for a Gosub-native package rather than a translation output.

4. **`clipboard.read` recorded as pure loss for this class.** Copying a password
   *for* the user needs `clipboard.write` only; the read side is a
   `user_text + credentials` source supporting a feature the vault does not
   need.
...
**OneTab (9)** is the minimal profile: tab metadata and private storage, zero page content access. It demonstrates that `tabs.metadata` should be a distinct capability from `tabs.content` — and, in v2.1 terms, why `tabs.restore(handle)` exists (§19): a tab manager holds `tabs.snapshot` (`source: tab_urls`) and needs to *reopen* the tabs it saved; if reopening meant `tabs.navigate(url)` (`sink: arbitrary_network`), the minimal profile would derive `history.exfiltration` ("can send the addresses of your tabs to any server") — true in the algebra, since a URL the extension composes can encode the tab list. With a browser-managed session store and opaque handles the browser chooses the navigation bytes, the extension never does, and OneTab's dialog stays clean *and* true. Plain `tabs.navigate` remains for extensions that genuinely need to open URLs they compose, with the derived warning that implies.
...
- scriptlets in the browser library -> `filtering.scriptlet` (loud until the O3 proofs, standard for a list confined to the Appendix D shortlist), no page access;
...
`filtering.dynamic_rules` was missing from earlier drafts of this grant, and the omission hid a problem: uBO's custom "My filters", the element picker's output, and the per-site on/off switch all install rules at runtime, which is exactly `dynamic_rules` (standard, `sink: probe + own_hosts`, §5). Under v2.1.9's `source: implicit_history` label that would have derived `history.exfiltration` against the `active_tab` sink — a scary first line on the flagship dialog; withdrawing the unsupported source label (§5) and keeping the real `own_hosts` sink leaves only the `!` line `active_tab` already earns.
...
Gained:    stats.display — the blocked-count badge that Chrome
           ...
           grant
Added:     filtering.dynamic_rules — custom filters / picker / per-site
           switch; standard; a rule-choice channel to the publisher's
           own observable servers (sink: own_hosts, §5), which for a
           publisher with no embedded web presence is an empty set
...
## Appendix C2: The Same Exercise, Backwards — a Password Manager

Appendix C asks what must change for uBO Lite to run on Gosub, and answers
*nothing*. This asks the opposite question of the extension §11 uses as its
model citizen, and the answer is less comfortable — which is the point of
including it.

§11 shows a password manager with **no `!` line at all**:
...
Translating the live Bitwarden manifest (MV3, 2026.7.0) produces **six**:

```text
Bitwarden wants to:
    ! Read and change the pages you visit          content_script(all sites)
    ! Watch every request your browser makes       network.observe(all sites)
    ! When you click its icon, read and change
      the current page                             content_script.active_tab
    ! See the address of every page you open       tabs.events
    ! Read your clipboard, including anything
      you copied                                   clipboard.read
    ! Answer password prompts from websites
      on your behalf                               network.auth_provider
    ✓ Contact the network, store data, show a toolbar button, a panel,
      notifications, a context menu, a badge, respond to shortcuts,
      put things on your clipboard, see whether you are at your computer
    ✗ It cannot record your screen
    ✗ It cannot read your device's details
```

Derived: `credential.exfiltration`, `history.exfiltration`, `page.exfiltration`,
`query.exfiltration`, and eight Axis-2 control products.

**Neither dialog is wrong.** The §11 example is what a password manager looks
like *rebuilt on the primitives in §19*; the second is what today's extension
translates to. The distance between them is the clearest single statement of
what this model is for, so it is worth being precise about where it comes from.

### Where the six lines come from

The manifest asks for `scripting` + `<all_urls>`, `tabs`, `webNavigation`,
`webRequest`, `webRequestAuthProvider`, `clipboardRead`. Every one is load-
bearing for a feature users want, and every one is answered by a §19 primitive
that does not require the broad grant:

```text
what it needs                    what it asks Chrome for   what §19 offers
------------------------------   -----------------------   ------------------
find the login form              content_script(<all>)     forms.detect_credentials
fill it                          content_script(<all>)     forms.fill
know which site you are on       tabs + webNavigation      (implied by fill scope)
notice a login to offer to save  webRequest(<all>)         forms.detect_credentials
answer HTTP auth prompts         webRequestAuthProvider    (no primitive — see below)
copy a password for you          clipboardRead/Write       clipboard.write alone
sync the vault                   host_permissions(<all>)   network.egress_public(vault host)
```

The substitution is not cosmetic. `forms.fill` is `standard` **only** because
the browser owns the two facts that would otherwise make it dangerous: each
fill needs a fresh browser-rendered gesture the extension cannot synthesize,
and the extension cannot CHOOSE the credential — the browser does (§19). A
content script that types into the page has neither property, which is why the
same user-visible feature costs `page_content × arbitrary_network` there and
nothing here.

`clipboard.read` is the sharpest case, because it is pure loss: an extension
that copies a password *for* you needs only `clipboard.write`. Reading is a
`user_text + credentials` source, and it is in the manifest to support
"detect what the user copied", a feature the vault does not need.

### The gap this exercise exposed, and its answer

The first draft of this appendix ended here, with `webRequestAuthProvider` as
the one row of the table with no mediated column: answering HTTP Basic/Digest
challenges had a §19 entry (`network.auth_provider`, loud — an
`implicit_history` source AND an `arbitrary_network` sink) and no alternative,
so a password manager supporting HTTP auth could not avoid it.

`forms.fill_auth(origins)` (§19, standard) closes it by applying the
`forms.fill` pattern to the browser's auth prompt instead of to a page form.
The substitution table's last row therefore reads:

```text
answer HTTP auth prompts       webRequestAuthProvider    forms.fill_auth(origins)
```

Both of `network.auth_provider`'s labels come from the extension being in the
loop, and both disappear when it is not. The `implicit_history` source is
*which origins issued challenges* — a browsing fact the extension learns only
because it is handed the event. The `arbitrary_network` sink is the credential
bytes: the extension chooses the username string sent to any origin it can
provoke into challenging. Under `forms.fill_auth` the browser renders the
prompt, selects candidates from the origin-bound store, and the user confirms;
the extension supplies no bytes and is told nothing.

That "told nothing" is a design requirement rather than a happy consequence,
and §19 states it as one: **no per-challenge feedback reaches the extension —
not success, not failure, not a count.** A build that reports fills back for
accounting or UI has re-created the source, and is loud again.

### Why this belongs next to Appendix C

Appendix C shows the model imposing no cost on a well-behaved declarative
extension. This shows the model imposing a large, visible cost on a
well-intentioned one that was written against a different set of primitives —
and shows that cost is *legible* rather than hidden, which is the property §11
exists to provide. A user comparing two password managers on Gosub would see
six warnings against none, and that difference would mean something specific.
...
- **The catalogue confirms G2 is a fact, not an idealization.** Untrusted `set-constant` (and the storage setters) ship with a *restricted* value vocabulary that rejects out-of-set values — arbitrary strings/URLs live only in the `trusted-*` family, which is exactly the set Appendix D excludes by construction. So the fixed-enum premise the whole lemma rests on is how the real library already behaves; the browser library enforces the same clamp and routes any arbitrary-value or `trusted-*` operator loud.
- **Cookies are loud, `localStorage` is not — do not conflate the two storage writes.** `set-cookie` is a `session_state` delayed-egress sink (§5): the browser auto-emits the written cookie to a server, so even a fixed-enum cookie value is loud. `set-local-storage-item` writes to a store the page reads but the browser never auto-transmits, so a fixed-enum value is integrity-only and standard-eligible. The distinction is the emission channel, not the value.
- **Caveat — verify value-sets against the live upstream before shipping.** This classification is against the `AdGuard/Scriptlets` catalogue as known; G2 is only as strong as the value-set the browser library actually clamps to, and vendors occasionally widen a vocabulary. The launch library must re-confirm each standard-eligible operator's value-set against the current upstream source and pin it, per the O8 proof-carrying obligation.
