...
actuator       authority the extension can DRIVE
               ...
               browser_ui | session_state | os
```

These enums are the **authoritative label algebra**, and the registry (§19) is mechanically validated against them — no registry entry may use a label absent here, and the closure (Axis 1 × Axis 2) is defined over exactly this vocabulary. Earlier drafts let §19 introduce atoms (`probe`, `download_urls`, `user_text`, `browser_traffic`, `native_process`, `enterprise_policy`, `publisher_update`) the §5 enums did not list; that is a hole, because "capabilities added later compose automatically" is only true if every atom has a defined product with every sink/actuator. The vocabulary is now closed and the validation is a build step, not prose.
...
non-interference MOVEMENT of bytes is necessary but NOT sufficient: a
              ...
              on page state. Operators are admitted only where their
              page-dependent branching stays confined to the page's own
              realm — which the audit checks per operator, since it is
              not decidable from arity/type alone
...
in-flight    every privileged operation is tagged at validation
             time with the grant epoch and re-checked against the
             live epoch at the point of effect; an operation whose
             epoch no longer matches is aborted, not completed — so
             a long-running or already-validated operation cannot
             race the revocation, and a request already matched
             against the old table does not get to finish under the
             withdrawn grant
...
cookies.write(hosts)                  loud   I:high; sink: session_state
...
Still open:
  ...
  O3  The exact procedural DSL operator set at launch (criterion in §8),
      ...
      data-write schema cannot cover them); the trusted-* family is out
      by construction. Discharging these per-operator proofs is also
  ...
  O5  stats privacy-budget parameters (criterion: a single-site probe
      ...
      against a GLOBAL per-profile pool so sharding across many
      one-rule extensions cannot parallelize past the bound (§7).
  ...
  O7  Covert-channel review (storage/cache/DNS/timing/quota) — the one
      ...
      scripts (§8) — which the §5 closure does NOT enumerate because it
      is an undeclared cross-publisher channel, so it is handled here by
      review, not by the composition detector — and rewrite_url's
      destination-server observability (§6).
