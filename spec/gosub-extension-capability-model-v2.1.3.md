...
write targets property-path arguments may not reach the navigation /
              cookie / network-loading families — location.*,
              document.cookie, window.open, src/href/action setters —
              and may not traverse __proto__ / constructor /
              prototype. Without this, set-constant('location.href',
              evil) is a standard-tier remote redirect and the honest
              actuator label would be navigation, not dom
...
responses     FIXED ENUM (empty object / array / string, noop js) —
              ...
              no trusted-* family)
channel       NONE from the injected realm to ANY extension context —
              not the worker, and not an isolated-world content
              script sharing the page DOM: no operator copies page-
              derived data into DOM attributes, globals, or any
              location a co-resident extension context can read.
              Scriptlets change the page; they never report on it
...
filtering.rewrite_url(class, hosts)   standard, ALL classes — no silent
                                      ...
                                      duplicates all removed; scheme/host/
                                      port/path/fragment/userinfo byte-
                                      identical; no insertion or
                                      substitution at any tier; no
                                      feedback to the extension;
...
filtering.scriptlet(hosts)            standard; browser-supplied audited
                                      ...
                                      NO channel to ANY extension context
                                      (worker or isolated-world script),
                                      cost-bounded against page input;
                                      injected by the browser at document-
                                      start into the page main world,
                                      frame-epoch bound (§13);
...
Still open:
  ...
  O3  The exact procedural DSL operator set at launch (criterion in §8),
      ...
      bounds); the trusted-* family is out by construction; library
      version semantics ride browser releases (§8).
