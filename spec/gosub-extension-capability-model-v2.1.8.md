...
transitive    and the reverse direction too: an operator's write must
              ...
              TRANSITIVE-effect proof, and there is no cheaper
              substitute: "restrict operators to suppressive semantics"
              looks like one but is UNSOUND — suppressing a page's own
              guard ENABLES the guarded action (no-op a paywall check
              ...
              code-runs is not monotonic in effects. Until the
...
Still open:
  ...
  O3  The exact procedural DSL operator set at launch (criterion in §8),
      ...
      because even a vocabulary-safe write can drive a page gadget; the trusted-* family is out by construction. The semantic
  ...
  O7  Covert-channel review (storage/cache/DNS/timing/quota) — the one
      ...
      set and stated honestly as ESSENTIALLY OPEN between any two
      extensions reaching the same page realm (name randomization is
      defeated by global/DOM enumeration; the content-script DOM
